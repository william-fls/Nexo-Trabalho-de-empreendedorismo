...
- `js/store.js` — defines `window.NexoStore` (`load/save/reset`, `uid`, `dayPlus`), seed data, `localStorage` key `nexo_db_v1` (`v:13`; `load()` migrates `v:1–v:12` bases, backfills storefront/shop fields, product `cat` (restoring seed cats when missing or `Geral`), `menuCats`, missing demo shops, the demo pessoa-física account, `request.prestId` (null = legacy open request), `cart`/`orders`, default `disponibilidade`, and renames the city to fictional Vila Aurora).
